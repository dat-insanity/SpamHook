...
amount = \
    int(input('\33[31mHow many times should the message send (input a negative number for unlimited): '
        ))
option = \
    input('\33[31mWhat mode do you pick, custom message (1), ascii spammer (2), or wordlist spammer (3): '
          )
...
if option == '1':
    ...
    while amount != 0:
        data = {'content': ascii(1999), 'username': username}
        result = requests.post(webhook, json=data)
        try:
            result.raise_for_status()
        except requests.exceptions.HTTPError, err:
